...
public struct SignoffMenuContent: View {
    ...
    /// The two-column main surface with a slim status footer underneath.
    private var mainSurface: some View {
        ...
        .frame(minWidth: 520, idealWidth: 580, maxWidth: 700, minHeight: 480)
        .background(popoverGlass)
        .task {
    ...
    private var voicesColumn: some View {
        VStack(alignment: .leading, spacing: Brand.Layout.spacingS) {
            ...
            Spacer(minLength: Brand.Layout.spacingS)
        }
    ...
    // MARK: - Footer bar (privacy / provider / History / Settings / Quit)

    /// A single, slim footer row carrying everything that is a *utility* rather
    /// than *content*. Keeping it in one full-width bar (vs. tucked under the
    /// voice column) lets the columns stay clean and gives the privacy promise
    /// a permanent, glanceable home — the thing that makes this feel trustworthy.
    private var footerBar: some View {
        ...
        .background(Brand.Surface.card(for: scheme).opacity(scheme.isDark ? 0.4 : 0.5))
...
private struct SplashOverlay: View {
    ...
    var body: some View {
        ...
        // Match the popover's ideal size so the symbol centers correctly
        .frame(minWidth: 520, idealWidth: 580, maxWidth: 700, minHeight: 480, idealHeight: 520)
        .zIndex(2)
        .transition(.opacity)
        .onAppear {
            ...
            // Phase 2: hold written (0.1s)
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                splashPhase = .written
                drawOnTrigger = false  // Reset so it can re-trigger if needed
            }

            // Phase 3: unwriting (0.5s)
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
                splashPhase = .unwriting
                drawOffTrigger = true
            }

            // Phase 4: fading out (0.2s)
