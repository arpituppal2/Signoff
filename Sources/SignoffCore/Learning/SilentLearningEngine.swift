...
public final class SilentLearningEngine: ObservableObject {
    ...
    // Callback is @convention(c) so it runs on the main run loop but isn't @MainActor
    // We just pass the notification string and let @MainActor process it
    private static let notificationQueue = DispatchQueue(label: "signoff.ax.notifications", qos: .userInitiated)
    ...
    private func _processPendingNotifications() {
        let pending = Self.notificationQueue.sync {
            let p = pendingNotifications
            pendingNotifications.removeAll()
            return p
        }
    ...
    private func handleAXNotificationRaw(notification: String, element: AXUIElement) {
        Self.notificationQueue.async {
            self.pendingNotifications.append((notification, element))
        }
        // Kick the main run loop to process
