...
public final class PasteAutomation: ObservableObject, @unchecked Sendable {
    ...
    /// Tuned above a bare `Task.yield()` — 100 ms races Mail/Slack/Electron
    /// paste consumers; 200 ms is the floor that keeps restore off the ⌘V path.
    /// Tests may zero this out.
    var pasteboardRestoreDelayNanoseconds: UInt64 = 200_000_000

    /// Success-path side effect (celebration capsule lives in SignoffUI).
    /// SignoffApp wires `CelebrationCapsuleCoordinator` at launch; unit tests
    /// leave the no-op so they don't pay the ~2s capsule decay sleep.
    ...
    public func paste(_ text: String) async throws {
        ...
        pb.clearContents()
        let wroteOK = pb.setString(text, forType: .string)
        guard wroteOK else {
