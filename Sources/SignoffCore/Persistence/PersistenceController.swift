...
public final class PersistenceController: @unchecked Sendable {
    ...
    public func slimBucketsIfNeeded() throws {
        ...
        for bucket in all {
            ...
            case BucketID.standard.rawValue:
                if bucket.name != "Normal" { bucket.name = "Normal"; changed = true }
                if bucket.sortOrder != 0 { bucket.sortOrder = 0; changed = true }
                if !bucket.isEnabled { bucket.isEnabled = true; changed = true }
            case BucketID.professional.rawValue:
                if bucket.name != "Professional" { bucket.name = "Professional"; changed = true }
                if bucket.sortOrder != 1 { bucket.sortOrder = 1; changed = true }
                if !bucket.isEnabled { bucket.isEnabled = true; changed = true }
    ...
    public func saveProfile(_ profile: UserProfile) throws {
        if let existing = try fetchProfile() {
            ...
            existing.footerFieldsRaw = profile.footerFieldsRaw
    ...
    /// v3: delete a single history entry.
    public func deleteGeneration(_ generation: SignoffGeneration) throws {
    ...
    /// favorites are always kept. Safe to call on every launch / refresh.
    public func pruneGenerations(olderThanDays days: Int = 3) throws {
        let cutoff = Date().addingTimeInterval(-Double(days) * 86_400)
        var descriptor = FetchDescriptor<SignoffGeneration>(
            predicate: #Predicate { $0.createdAt < cutoff && !$0.isFavorite })
