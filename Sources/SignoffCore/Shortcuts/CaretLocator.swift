...
public final class CaretLocator {
    ...
    private func elementFrame(of element: AXUIElement) -> CGRect {
        ...
        var posValue: AXValue?
        var sizeValue: AXValue?
        if AXUIElementCopyAttributeValue(element, kAXPositionAttribute as CFString, &posRef) == .success, posRef != nil {
            posValue = posRef! as! AXValue
        }
        if AXUIElementCopyAttributeValue(element, kAXSizeAttribute as CFString, &sizeRef) == .success, sizeRef != nil {
            sizeValue = sizeRef! as! AXValue
        }
        var position = CGPoint.zero
        var size = CGSize.zero
        if let pv = posValue { _ = AXValueGetValue(pv, .cgPoint, &position) }
        if let sv = sizeValue { _ = AXValueGetValue(sv, .cgSize, &size) }
