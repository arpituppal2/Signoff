...
final class GenerationTests: XCTestCase {
    ...
    func testProviderKind_IsOnDeviceAI() {
        XCTAssertEqual(GenerationProviderKind.foundationModels.badgeTitle, "On-device AI")
    ...
    func testProviderKind_OfflineFallbackBadgeIsHonest() {
        // Offline phrasebook must never be relabeled as AI.
        XCTAssertEqual(GenerationProviderKind.offlineFallback.badgeTitle, "Offline phrasebook")
        XCTAssertEqual(GenerationProviderKind.offlineFallback.rawValue, "offline")
        XCTAssertFalse(GenerationProviderKind.offlineFallback.isLiveOnDevice)
        XCTAssertTrue(GenerationProviderKind.foundationModels.isLiveOnDevice)
    }

    func testProviderKind_AllCases() {
        let all = GenerationProviderKind.allCases
        XCTAssertEqual(all, [.foundationModels, .offlineFallback])
    }
