...
public final class CarbonEventTap: @unchecked Sendable {
    ...
    public func start() -> Result<Void, TapFailure> {
        ...
        // Input Monitoring is required before CGEvent.tapCreate for global
        // key listening. Prompt explicitly so failure copy can blame the
        // correct Privacy pane (not Accessibility).
    ...
    public static func canTap(keyCode: Int32, modifiers: UInt32) -> Bool {
        _ = keyCode
        _ = modifiers
        guard InputMonitoringAccess.isGranted() else { return false }
